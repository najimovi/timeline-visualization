 ...
 */
export const useTimelineLayout = ({ items, zoomLevel, colors }: UseTimelineLayoutProps): ProcessedItem[] => {
  return useMemo(() => {
    ...
    for (const item of itemsWithDates) {
      ...
      const duration = Math.max(item.endDate.getTime() - item.startDate.getTime(), 86400000) // Minimum 1 day
      const position = (item.startDate.getTime() - minDate.getTime()) / totalDuration
      ...
      for (let laneIndex = 0; laneIndex < lanes.length; laneIndex++) {
        ...
        for (const existingItem of lane) {
          ...
          if (!(position >= existingEnd + buffer || newEnd + buffer <= existingItem.position)) {
            canFit = false
            break
          }
        }
        ...
        if (canFit) {
          ...
          if (!textWillFit && item.name.length > 20 && laneIndex < 3 && lanes.length < 10) {
