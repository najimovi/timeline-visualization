 ...
 */
export const useTimeMarkers = ({ processedItems, zoomLevel }: UseTimeMarkersProps): TimeMarkersResult => {
  return useMemo(() => {
    ...
    while (monthCurrent <= maxDate) {
      const position = ((monthCurrent.getTime() - minDate.getTime()) / totalDuration) * 100
      monthMarkers.push({
    ...
    while (dayCurrent <= maxDate) {
      const position = ((dayCurrent.getTime() - minDate.getTime()) / totalDuration) * 100
      dayMarkers.push({
